...
function focusInputGroupControl(event: React.SyntheticEvent<HTMLFieldSetElement>) {
...
function InputGroupAddon({
  ...
  return (
    <fieldset
      ...
      data-align={align}
      className={cn("min-w-0 border-0 p-0", inputGroupAddonVariants({ align }), className)}
      onClick={focusInputGroupControl}
      onKeyDown={(event) => {
        if (event.key === "Enter" || event.key === " ") {
          focusInputGroupControl(event);
        }
      }}
      {...props}
    />
